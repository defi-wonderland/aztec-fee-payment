 ...
 */
const DEFAULT_CHAINS: Record<number, Partial<ChainConfig>> = {
  // Ethereum Mainnet
  1: {
...
function getDefaultChainName(chainId: number): string {
  return DEFAULT_CHAINS[chainId]?.name ?? `Chain ${chainId}`;
}

function getDefaultConfirmations(chainId: number): number {

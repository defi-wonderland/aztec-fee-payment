...
export async function maybeUploadArtifactToRegistry(params) {
  ...
  try {
    ...
    console.warn(
      `[artifact-registry] Upload failed (continuing): ${err instanceof Error ? err.message : String(err)}`,
    );
    return null;
  }
}
...
export async function loadArtifactWithRegistryFallback(params) {
  ...
  if (params.classId) {
    try {
      ...
      console.warn(
        `Failed to fetch artifact from registry, using local file: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }

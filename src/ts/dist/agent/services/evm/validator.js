...
export async function validateTransaction(options) {
    const { client, txHash, from, feeCollectorAddress, aztTokenAddress, requiredConfirmations, minAmount, logger, } = options;
    // 1. Fetch receipt
    ...
    if (confirmations < requiredConfirmations) {
        throw invalidInput("TX_NOT_FINALIZED", `Transaction has ${confirmations} confirmations, required ${requiredConfirmations}`, { confirmations, required: requiredConfirmations });
    }
    // 4. Parse transfers and filter
    const allTransfers = parseTransferEvents(receipt);
    const matchingTransfers = findFeeCollectorTransfers(allTransfers, feeCollectorAddress, aztTokenAddress, from);
    ...
    if (totalAmount < minAmount) {
        throw invalidInput("INVALID_AMOUNT", `Transfer amount ${totalAmount} is below minimum ${minAmount}`, {
            amount: totalAmount.toString(),
            minAmount: minAmount.toString(),
        });
    }
    logger.info({ txHash, amount: totalAmount.toString(), from }, "Transaction validated");

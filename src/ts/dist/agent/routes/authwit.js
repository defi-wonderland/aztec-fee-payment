...
export function createAuthwitRouter(deps) {
    const { config, evmClients, secretGenerator, authwitGenerator, logger } = deps;
    const router = Router();
    ...
    router.post("/authwit/request", validate, async (req, res, next) => {
        ...
        try {
            ...
            if (!client || !chainConfig) {
                throw invalidInput("INVALID_CHAIN", `Chain ID ${body.evmChainId} is not supported`, { supportedChains: evmClients.getSupportedChains() });
            }
            // 2. Recover sender from EIP-712 signature
            reqLogger.info("Recovering EIP-712 signer");
            let from;
            try {
                from = await recoverClaimRequestSigner({ txHash: body.evmTxHash }, body.signature, body.evmChainId);
            }
            catch {
                throw invalidInput("INVALID_SIGNATURE", "Could not recover signer from EIP-712 signature");
            }
            ...
            const authwit = await authwitGenerator.generateMintAuthwit(txResult.amount, secret);
            // 6. Return response
            reqLogger.info({ amount: txResult.amount.toString() }, "Authwit generated");

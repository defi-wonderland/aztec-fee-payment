...
11. **Signing key rotation**: See [Signing Key Rotation](#signing-key-rotation) below.

### Signing Key Rotation

The contract supports owner rotation via `update_owner()`.

Rotating `SP_SIGNING_KEY` requires coordinating the off-chain agent with the on-chain `DelayedPublicMutable` owner transition. The core constraint is that the agent is **stateless**: it cannot know whether a user already minted with a previous secret. If the same TopUp event ever produces two different secrets (one per key), the resulting authwits yield different nullifiers (`H(amount, secret_old)` vs `H(amount, secret_new)`), and both could be used to mint.

**Procedure**:

1. Call `update_owner(newOwner)`. This schedules the change; the new owner becomes effective only after `CONFIG_DELAY` (~600 blocks).
2. **Cutoff rule**: from the moment `update_owner` is called (the _schedule block_, not the later block where the change becomes effective), the agent starts using the new key for any TopUp event emitted at or after that block. TopUp events emitted before the schedule block continue using the old key, permanently. This deterministic binding prevents double-mint.
3. Authwits generated with the new key will **not work** until `CONFIG_DELAY` elapses and the new owner becomes effective on-chain. Users who top up during this window must wait before minting.

**Trade-off — stranded authwits**: Old-key authwits for pre-cutoff TopUp events become permanently invalid once the delay elapses. Users who paid AZT but didn't claim in time cannot mint via the agent, and the SP has no way of knowing which users are affected. Mitigations:

- Announce the rotation window in advance so users claim pending authwits before the deadline
- After the delay, the agent should detect expired old-key requests (TopUp block < schedule block AND current block > schedule block + `CONFIG_DELAY`) and return an explicit error (e.g. `AUTHWIT_EXPIRED_KEY_ROTATION`) instead of a silently-failing authwit
- In the current implementation, the agent could compute the authwit nullifier and check on-chain whether it was already consumed, distinguishing claimed from stranded events. If this remains true in the future, it could be used to prevent stranded aithwits completely.
